...
class JsonRepair
{
    ...
    private const STATE_IN_STRING = 1;
    private const STATE_IN_STRING_ESCAPE = 2;
    private const STATE_IN_NUMBER = 3;
    private const STATE_IN_OBJECT_KEY = 4;
    private const STATE_IN_OBJECT_VALUE = 5;
    private const STATE_IN_ARRAY = 6;
    private const STATE_EXPECTING_COLON = 7;
    private const STATE_EXPECTING_COMMA_OR_END = 8;
    ...
    public function repair(): string
    {
        ...
        while (!empty($this->stack)) {
            ...
            // If we're closing an object and output ends with ':', add empty string
        ...
        // Apply ensureAscii if needed
        if (!$this->ensureAscii) {
            // Decode and re-encode to preserve non-ASCII characters
        ...
        if (json_validate($this->output)) {
            return $this->output;
        }

        // If still invalid, return what we have
        return $this->output;
    }
    ...
    private function extractJsonFromMarkdown(string $input): string
    {
        ...
        // Extract JSON from code blocks without language
    ...
    private function extractFirstValidJson(string $input): string
    {
        ...
        // Try to find the first valid JSON structure by checking from start
        // Look for objects first, then arrays
        $length = strlen($input);
        $bestMatch = null;
        $bestLength = 0;
            ...
            if ($char === '{') {
                ...
                if ($depth === 0 && $start !== -1) {
                    ...
                    if (json_validate($extracted)) {
                        // Prefer objects over arrays, and longer matches
                        if ($bestMatch === null || strlen($extracted) > $bestLength) {
                            $bestMatch = $extracted;
                ...
                if ($depth === 0) {
                    $extracted = substr($input, $start, $i - $start + 1);
                    if (json_validate($extracted)) {
                        // Only use array if no object found
                        if ($bestMatch === null) {
                            $bestMatch = $extracted;
                            $bestLength = strlen($extracted);
                        }
    ...
    private function handleObjectKey(string $json, int $i): int
    {
        ...
        if ($char === '}') {
            ...
            $this->state = empty($this->stack) ? self::STATE_START : self::STATE_EXPECTING_COMMA_OR_END;
            return $i + 1;
        }
    ...
    private function handleObjectValue(string $json, int $i): int
    {
        ...
        if ($char === '}') {
            // Missing value, insert empty string if we have a colon
            if (str_ends_with($this->output, ':')) {
                $this->output .= '""';
            }
            if (str_ends_with($this->output, ',')) {
        ...
        // Handle Python-style booleans/null
    ...
    private function handleNumber(string $json, int $i): int
    {
        ...
        $top = end($this->stack);
        $this->state = $top === '}' ? self::STATE_EXPECTING_COMMA_OR_END : self::STATE_EXPECTING_COMMA_OR_END;
